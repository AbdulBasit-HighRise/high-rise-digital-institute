...
import Image from "next/image"; // Crash-safe production image engine
...
export default function Navbar() {
  ...
  // Contentful REST Integration Engine (Bypasses crashes on build)
  useEffect(() => {
    if (!mounted) return;
  ...
  const links = [
    ...
    { 
      ...
      isDropdown: true,
      subLinks: [
        { name: "SEO Mastery", href: "/#courses" },
        { name: "WordPress Development", href: "/#courses" },
        { name: "E-Commerce Pipeline", href: "/#courses" }
      ]
    },
        ...
        >
                        ...
                        {showServices && (
                          <motion.div initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 5 }} className="absolute top-full left-0 w-56 p-2 bg-black border border-white/10 rounded-2xl shadow-2xl mt-1">
                            {link.subLinks?.map((sub: any) => (
                              <Link key={sub.name} href={sub.href} onClick={(e) => handleScrollToSection(e, sub.href)} className="block px-4 py-2.5 text-[13px] font-bold text-zinc-400 hover:text-white hover:bg-white/5 rounded-lg transition-all">
                                {sub.name}
                              </Link>
                            ))}
                          </motion.div>
          ...
          <div className="flex items-center gap-3 z-50 shrink-0">
            ...
            <button 
              ...
              className="group relative overflow-hidden hidden lg:flex items-center justify-center font-black text-[11px] uppercase tracking-[2px] h-[44px] px-6 rounded-full bg-zinc-900 text-white border border-white/10 shadow-lg transition-all duration-500 ease-out active:scale-95 cursor-pointer"
            >
            ...
            <button 
              ...
              className="lg:hidden text-white p-2 focus:outline-none rounded-xl bg-white/[0.02] border border-white/10 backdrop-blur-md animate-none"
            >
            ...
            {isOpen && (
              <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="absolute top-full left-0 right-0 bg-[#070707]/98 backdrop-blur-xl border border-white/10 lg:hidden flex flex-col gap-0 p-4 mt-2 rounded-3xl pointer-events-auto shadow-2xl overflow-hidden">
                ...
                <div className="pt-4 px-4 flex flex-col gap-3">
                  ...
                  <button 
                    onClick={() => { setIsOpen(false); setIsProposalOpen(true); }}
                    className="block text-center w-full bg-zinc-900 border border-white/10 text-white font-black text-xs uppercase tracking-wider py-3.5 rounded-xl shadow-lg"
                  >
                    Enroll Now
                  </button>
                </div>
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative w-full max-w-lg bg-[#070707] border border-white/10 rounded-[2.5rem] p-8 shadow-2xl">
              ...
              <form className="space-y-4" onSubmit={handleModalSubmit}>
                ...
                <select
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-xs text-zinc-400 outline-none focus:border-blue-500 transition-all disabled:opacity-40"
                >
                  <option value="" disabled className="bg-black text-zinc-500">Select Training Track</option>
                  <option value="seo" className="bg-black text-white">SEO Program</option>
                  <option value="wordpress" className="bg-black text-white">WordPress Masterclass</option>
                </select>
